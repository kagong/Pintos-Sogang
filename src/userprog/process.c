...
void pinfo_set_status(int status)
{
	...
	struct process_info *p = find_pinfo(thread_parent_child_list(),tid);
	p->status=status;
}
int pinfo_get_status(tid_t target)
{
	return find_pinfo(thread_child_list(),target)->status;
}
void free_pinfo()
{
...
start_process (void *file_name_)
{
	...
	hash_init(&thread_current()->spt,user_hash_func,user_hash_less,NULL);
	...
	sema_down(&thread_current()->test_sema);
...
setup_stack (void **esp) 
{
	...
	if (kpage == NULL)
	{
		kpage = evit_frame(PAL_USER | PAL_ZERO,((uint8_t *) PHYS_BASE) - PGSIZE);
		if(kpage == NULL){
			return false;
		}
	}
...
install_page (void *upage, void *kpage, bool writable)
{
	...
	return (pagedir_get_page (t->pagedir, upage) == NULL
			&& spt_add_page(upage, kpage, writable));
}
